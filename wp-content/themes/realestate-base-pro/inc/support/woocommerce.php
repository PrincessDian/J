...
class Realestate_Base_Woocommerce {
	...
	function init() {
		...
		// Sidebar filter.
		add_filter( 'realestate_base_filter_default_sidebar_id', array( $this, 'sidebar_defaults' ), 10, 2 );

		// Customizer options.
		add_action( 'customize_register', array( $this, 'customizer_fields' ) );

		// Add default options.
		add_filter( 'realestate_base_filter_default_theme_options', array( $this, 'default_options' ) );
	...
	function modify_global_layout( $layout ) {
		...
		$woo_page_layout = realestate_base_get_option( 'woo_page_layout' );
	...
	function customizer_fields( $wp_customize ) {
		...
		$wp_customize->add_section( 'section_theme_woocommerce',
			array(
				'title'       => esc_html__( 'WooCommerce Options', 'realestate-base-pro' ),
				'description' => esc_html__( 'Settings specific to WooCommerce. Note: WooCommerce Page means shop page, product page and product archive page.', 'realestate-base-pro' ),
				'priority'    => 100,
		...
		$wp_customize->add_control( 'theme_options[woo_page_layout]',
			array(
				'label'   => esc_html__( 'Content Layout', 'realestate-base-pro' ),
		...
		$wp_customize->add_control(
			new Realestate_Base_Dropdown_Sidebars_Control( $wp_customize, 'theme_options[woo_sidebar_primary]',
				array(
					'label'       => esc_html__( 'Primary Sidebar', 'realestate-base-pro' ),
					'description' => esc_html__( 'Choose Primary Sidebar for WooCommerce pages. If not selected default sidebar will be displayed.', 'realestate-base-pro' ),
					'section'  => 'section_theme_woocommerce',
		...
		$wp_customize->add_control(
			new Realestate_Base_Dropdown_Sidebars_Control( $wp_customize, 'theme_options[woo_sidebar_secondary]',
				array(
					'label'       => esc_html__( 'Secondary Sidebar', 'realestate-base-pro' ),
					'description' => esc_html__( 'Choose Secondary Sidebar for WooCommerce pages. If not selected default sidebar will be displayed.', 'realestate-base-pro' ),
					'section'  => 'section_theme_woocommerce',
	...
	function register_woo_sidebars() {
		...
		register_sidebar( array(
			'name'          => __( 'WooCommerce Primary', 'realestate-base-pro' ),
		...
		register_sidebar( array(
			'name'          => __( 'WooCommerce Secondary', 'realestate-base-pro' ),
	...
	function sidebar_defaults( $id, $location ) {
		...
		switch ( $location ) {
			case 'primary':
				$woo_sidebar_primary = realestate_base_get_option( 'woo_sidebar_primary' );
				if ( ! empty( $woo_sidebar_primary ) ) {
					$id = esc_attr( $woo_sidebar_primary );
				}
			break;
			case 'secondary':
				$woo_sidebar_secondary = realestate_base_get_option( 'woo_sidebar_secondary' );
