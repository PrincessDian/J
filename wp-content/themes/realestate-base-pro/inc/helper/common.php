	...
	function realestate_base_fonts_url() {
		...
		if ( ! empty( $font_settings ) ) {
			foreach ( $font_settings as $k => $v ) {
				$theme_options[] = realestate_base_get_option( $k );
			}
		}
		...
		if ( ! empty( $required_fonts ) ) {
			foreach ( $required_fonts as $key => $f ) {
				$fonts[] = $web_fonts[ $f ]['name'] . ':400,500,600,700';
			}
		}
...
if( ! function_exists( 'realestate_base_primary_navigation_fallback' ) ) :
	...
	function realestate_base_primary_navigation_fallback() {
		echo '<ul>';
		echo '<li><a href="' . esc_url( home_url( '/' ) ) . '">' . esc_html__( 'Home', 'realestate-base-pro' ). '</a></li>';
		wp_list_pages( array(
			'title_li' => '',
			'depth'    => 1,
			'number'   => 7,
	...
	function realestate_base_render_select_dropdown( $main_args, $callback, $callback_args = array() ) {
		...
		if ( ! empty( $choices ) || true === $r['add_default'] ) {
			...
			if ( true === $r['add_default'] ) {
				$output .= '<option value="">' . esc_html__( 'Default', 'realestate-base-pro' ) . '</option>\n';
			}
			if ( ! empty( $choices ) ) {
